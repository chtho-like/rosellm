...
if TRITON_AVAILABLE:
    ...
    def _kv_append_identity_pos_kernel(
        ...
        k_dst_off = blk * stride_kb + h * stride_kh + pos * stride_kt + d * stride_kd
        v_dst_off = blk * stride_vb + h * stride_vh + pos * stride_vt + d * stride_vd
        tl.store(k_cache_ptr + k_dst_off, k, mask=mask_d)
        tl.store(v_cache_ptr + v_dst_off, v, mask=mask_d)
...
def kv_append_triton_identity_pos(
    *,
    k_cache_layer: torch.Tensor,  # [N_BLOCKS, H, BS, D]
    v_cache_layer: torch.Tensor,  # [N_BLOCKS, H, BS, D]
    key_new: torch.Tensor,  # [B, H, D]
    ...
    if not (
        ...
        raise RuntimeError("kv_append_triton_identity_pos requires CUDA tensors")

    B = int(key_new.size(0))
    if B == 0:
        return
    if int(block_idx.numel()) != B or int(pos.numel()) != B:
        raise ValueError("block_idx/pos must have shape [B]")
    ...
    _kv_append_identity_pos_kernel[grid](
        k_cache_layer,
        v_cache_layer,
        key_new,
        value_new,
