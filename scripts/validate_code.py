...
class CodeValidator:
            ...
            if isinstance(node, ast.Import):
                ...
                for alias in node.names:
                    ...
                    if not self._can_import(module, alias.name):
                        import_errors.append(f"Cannot import '{alias.name}' from '{module}'")
    ...
    def _check_type_hints(self, filepath: Path) -> None:
        ...
        try:
            result = subprocess.run(
                ["mypy", str(filepath), "--ignore-missing-imports", "--no-error-summary"],
                capture_output=True,
                text=True,
                timeout=10
            )
    ...
    def _run_static_analysis(self, filepath: Path) -> None:
        ...
        try:
            result = subprocess.run(
                ["flake8", str(filepath), "--max-line-length=100", 
    ...
    def _check_common_issues(self, filepath: Path) -> None:
        ...
        if undefined_vars:
            issues.append(f"Potentially undefined variables: {', '.join(undefined_vars[:3])}")
    ...
    def _find_undefined_vars(self, tree: ast.AST) -> List[str]:
        ...
        builtins = {'True', 'False', 'None', 'int', 'str', 'float', 'list', 'dict', 
                   'set', 'tuple', 'print', 'len', 'range', 'open', 'super', 'self'}
