...
class MockOptimizer:
    ...
    def __init__(self):
        self.param_groups = [{"lr": 1e-3, "weight_decay": 0.01}]


def test_linear_decay(init_lr: float, max_lr: float, min_lr: float, steps: int) -> bool:
    """Test linear decay calculation."""
    # init_lr is unused in decay calculation - testing decay from max_lr to min_lr
    _ = init_lr
...
def test_cosine_decay(init_lr: float, max_lr: float, min_lr: float, steps: int) -> bool:
    ...
    for step in range(0, steps + 1, steps // 4):
        ...
        print(
            f"  Step {step:4d}: decay_ratio={decay_ratio:.2f}, "
            f"coeff={coeff:.4f}, lr={expected_lr:.6f}"
        )
...
def test_warmup(init_lr: float, max_lr: float, warmup_steps: int) -> bool:
    ...
    print(
        f"\nTesting linear warmup from {init_lr} to {max_lr} "
    ...
    for step in range(0, warmup_steps + 1, warmup_steps // 4):
        ...
        print(
            f"  Step {step:3d}: warmup_ratio={warmup_ratio:.2f}, lr={expected_lr:.6f}"
        )
...
def validate_all():
    ...
    # Test decay styles
    results.append(
        ("Linear Decay", test_linear_decay(init_lr, max_lr, min_lr, decay_steps))
    )
    results.append(
        ("Cosine Decay", test_cosine_decay(init_lr, max_lr, min_lr, decay_steps))
    )
    results.append(
        ("Inverse Sqrt", test_inverse_sqrt_decay(max_lr, min_lr, warmup_steps))
    )
    ...
    results.append(
        (
            "WSD Schedule",
            test_wsd_schedule(init_lr, max_lr, min_lr, warmup_steps, decay_steps, 200),
        )
    )
