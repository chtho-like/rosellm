...
def _kernel_sort_key(name: str) -> tuple[int, str]:
    if name.startswith("sgemm_"):
        rest = name[len("sgemm_") :]
        num = ""
        for ch in rest:
            if not ch.isdigit():
                break
...
def plot_kernel(
    ...
    ax.plot(sizes, cublas_tflops, marker="o", label="cuBLAS (FP32)")
    ax.plot(sizes, custom_tflops, marker="o", label=kernel)
    ax.set_title(f"SGEMM TFLOP/s vs size\n{gpu}")
    ...
    ax.set_title(f"SGEMM speedup vs cuBLAS\n{gpu}")
    ax.set_xlabel("Square size (M=N=K)")
...
def write_summary_md(out_dir: pathlib.Path, json_paths: list[pathlib.Path]) -> None:
    ...
    rows: list[str] = []
    header = (
        "| kernel | gpu | sizes | best speedup | best size | best custom TF |"
        " best cuBLAS TF |\n"
        "|---|---|---:|---:|---:|---:|---:|\n"
    )
    for p in json_paths:
...
def write_best_of_md(out_dir: pathlib.Path, json_paths: list[pathlib.Path]) -> None:
    ...
    fig = plt.figure(figsize=(9, 5))
    ax = fig.add_subplot(1, 1, 1)
    ax.plot(sizes, best_cublas, marker="o", label="cuBLAS (FP32)")
    ax.plot(sizes, best_custom, marker="o", label="best custom")
    title_gpu = f"\n{gpu}" if gpu else ""
    ax.set_title(f"Best SGEMM TFLOP/s vs size{title_gpu}")
    ax.set_xlabel("Square size (M=N=K)")
    ax.set_ylabel("TFLOP/s (higher is better)")
    ax.grid(True, alpha=0.3)
    ax.legend()
    ...
    ax.axhline(1.0, linestyle="--", color="black", linewidth=1)
    ax.set_title(f"Best SGEMM speedup vs cuBLAS{title_gpu}")
    ax.set_xlabel("Square size (M=N=K)")
    ax.set_ylabel("Speedup (1.0 == cuBLAS)")
    ax.grid(True, alpha=0.3)
    ax.legend()
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Build all sgemm_*.cu programs, run them, and plot vs cuBLAS."
        )
    )
    ...
    p.add_argument(
        ...
        default="all",
        help=(
            "Which kernels to run. Use 'all' or a CSV list of binary names, "
            "e.g. sgemm_5_double_buffered,sgemm_13_db_b256n128_bk16."
        ),
    )
    p.add_argument(
        "--sizes",
    ...
    p.add_argument(
        ...
        default="",
        help=(
            "Output dir for json + plots. Default is results/run_<ts>/."
        ),
    )
