...
File: transpose_2_tiled_no_bank_conflict.cu
Focus:
  - Same tiled transpose as transpose_1.
  - Add +1 padding in shared memory to avoid bank conflicts when reading
    the transposed tile: tile[32][33].
...
Memorize (recommended):
  - YES. The "[32][33]" trick is a famous CUDA interview detail.

Build (recommended for RTX 4070 / SM89):
  nvcc -O3 -std=c++17 -lineinfo -arch=sm_89 transpose_2_tiled_no_bank_conflict.cu \
    -o transpose_2_tiled_no_bank_conflict
Run:
  ./transpose_2_tiled_no_bank_conflict [rows] [cols] [device_id]
Example:
  ./transpose_2_tiled_no_bank_conflict 1024 1024 0
*/
...
__global__ void TransposeKernelTiledNoBankConflicts(const float* __restrict__ a,
...
int main(int argc, char** argv) {
  ...
  TransposeKernelTiledNoBankConflicts<<<grid, block>>>(d_a, d_b, rows, cols);
