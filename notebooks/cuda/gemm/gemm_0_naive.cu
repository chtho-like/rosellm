...
Focus:
  ...
  - Correctness first: understand indexing, layout, and bounds.

Interview tags:
  - Classic:     Very High
  - Importance:  Extremely High (LLM inference is GEMM everywhere)
  - Frequency:   Very High

Memorize:
  - Not the final version. Use this to build the mental model.

Build (recommended for RTX 4070 / SM89):
  nvcc -O3 -std=c++17 -lineinfo -arch=sm_89 gemm_0_naive.cu -o gemm_0_naive
Run:
...
Typical interviewer follow-ups:
  ...
  - Split-K for better occupancy / parallelism.
  - Tensor cores (WMMA / MMA), then Hopper WGMMA.
*/
