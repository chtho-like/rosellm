...
File: reduce_1_sequential_halving.cu
Focus: Remove expensive modulo + reduce divergence by switching to
       sequential addressing (stride halves each iteration).

Typical interviewer follow-up after reduce_0:
  - "Can you avoid the modulo and make whole warps active longer?"

Build:
  nvcc -O3 -std=c++17 -lineinfo reduce_1_sequential_halving.cu \
    -o reduce_1_sequential_halving
Run:
  ./reduce_1_sequential_halving [num_elements] [device_id]
Example:
  ./reduce_1_sequential_halving 16777216 0
Notes:
...
// Step 1 kernel:
// - Sequential addressing: active threads are tid < stride.
// - Upside: no modulo, less divergence, and fewer bank-conflict pitfalls.
