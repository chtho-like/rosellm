...
File: reduce_6_cub_blockreduce.cu
Focus: Use a production-ready primitive: cub::BlockReduce.

Typical interviewer follow-up after reduce_5:
  - "If you were writing production code, would you still hand-roll the
    block reduction? What would you use instead?"
  - A common answer: CUB (now part of CCCL shipped with CUDA).

Build:
  nvcc -O3 -std=c++17 -lineinfo reduce_6_cub_blockreduce.cu \
    -o reduce_6_cub_blockreduce
Run:
  ./reduce_6_cub_blockreduce [num_elements] [device_id]
Example:
  ./reduce_6_cub_blockreduce 16777216 0
Notes:
...
// Step 6 kernel:
// - Keep the same grid-stride + x4 loads as step 5.
// - Replace the hand-written block reduction with cub::BlockReduce.
__global__ void ReduceKernel(const float* __restrict__ in,
  ...
  const float block_sum = BlockReduce(temp_storage).Sum(sum);
  if (tid == 0) {
