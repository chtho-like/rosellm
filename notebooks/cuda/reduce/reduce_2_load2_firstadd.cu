...
File: reduce_2_load2_firstadd.cu
Focus: Reduce global memory traffic by loading 2 elements per thread and
       doing the "first add" during the load.

Typical interviewer follow-up after reduce_1:
  - "Can you reduce the number of blocks and global reads?"
  - Hint: each thread can load multiple elements, sum in registers, then
    reduce within the block.

Build:
  nvcc -O3 -std=c++17 -lineinfo reduce_2_load2_firstadd.cu \
    -o reduce_2_load2_firstadd
Run:
  ./reduce_2_load2_firstadd [num_elements] [device_id]
Example:
  ./reduce_2_load2_firstadd 16777216 0
Notes:
...
// Step 2 kernel:
// - Each thread loads 2 values (if in-bounds) and adds them in registers.
// - Block reduces these partial sums in shared memory.
