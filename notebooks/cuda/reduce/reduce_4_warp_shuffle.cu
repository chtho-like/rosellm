...
File: reduce_4_warp_shuffle.cu
Focus: Use warp shuffle intrinsics to finish the last warp in registers
       (avoid volatile shared-memory reads).

Typical interviewer follow-up after reduce_3:
  - "Can you avoid shared-memory traffic in the last warp?"
  - Hint: warp shuffle (e.g., __shfl_down_sync).

Build:
  nvcc -O3 -std=c++17 -lineinfo reduce_4_warp_shuffle.cu \
    -o reduce_4_warp_shuffle
Run:
  ./reduce_4_warp_shuffle [num_elements] [device_id]
Example:
  ./reduce_4_warp_shuffle 16777216 0
Notes:
...
// Step 4 kernel:
// - Reduce in shared memory down to 64 values.
// - Combine the last 64 -> 1 using warp shuffle in registers.
