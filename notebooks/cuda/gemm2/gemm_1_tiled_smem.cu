...
Build:
  nvcc -O3 -std=c++17 -lineinfo -arch=sm_80 gemm_1_tiled_smem.cu \
    -o gemm_1_tiled_smem
Run:
...
Typical interviewer follow-ups:
  - Thread tiling (8x8 per thread) + vectorized loads/stores (float4).
  - Bank-conflict mitigation (padding / layout transforms).
  - Double-buffering + cp.async (SM80+).
