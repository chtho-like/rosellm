...
File: gemm_3_smem_padding_bcf.cu
Focus:
  - Reduce shared-memory bank conflicts by changing shared-memory layout:
    - Store A tile as [BK][BM + PAD] (a.k.a. transpose A tile in SMEM).
    - Add padding (PAD) to break bad strides.
  - Keep the same 8x8 thread tile idea as gemm_2, but map each thread's
    outputs into 4 quadrants so SMEM loads become vector-friendly.

Key idea:
  - SMEM is fast only if a warp hits different banks.
  - A tiny "layout tweak" can save many SMEM transactions.

Build:
  nvcc -O3 -std=c++17 -lineinfo -arch=sm_80 gemm_3_smem_padding_bcf.cu \
    -o gemm_3_smem_padding_bcf
Run:
  ./gemm_3_smem_padding_bcf [M] [N] [K] [device_id]
Example:
  ./gemm_3_smem_padding_bcf 1024 1024 1024 0

Notes:
  - This kernel uses float4 LD/ST, so it requires N % 4 == 0 and K % 4 == 0.
  - Later files add double-buffering and cp.async on top of this layout.
*/
...
__global__ void GemmKernelSmemPadBcf(const float* __restrict__ a,
                                     const float* __restrict__ b,
                                     float* __restrict__ c,
                                     int m,
                                     int n,
                                     int k) {
  // A in SMEM: [BK][BM + PAD] (transposed).
  __shared__ float s_a[kBK][kBM + kPad];
  // B in SMEM: [BK][BN + PAD] (row-major with padding).
  __shared__ float s_b[kBK][kBN + kPad];
...
#pragma unroll
  ...
  // Map 256 threads to:
  // - A tile: 128 rows, each row loads 8 values => 2 float4 per row.
  // - B tile: 8 rows, each row loads 128 values => 32 float4 per row.
  ...
  for (int k0 = 0; k0 < k; k0 += kBK) {
    // Load A: gmem -> reg(float4) -> smem(transposed).
    {
      ...
      s_a[load_a_k4 + 0][load_a_row] = v.x;
      s_a[load_a_k4 + 1][load_a_row] = v.y;
      s_a[load_a_k4 + 2][load_a_row] = v.z;
      s_a[load_a_k4 + 3][load_a_row] = v.w;
    }

    // Load B: gmem -> smem (vectorized).
    {
      ...
      StoreFloat4(&s_b[load_b_k][load_b_col4], v);
    }

    __syncthreads();

    // Thread outputs:
    // - Rows: 4 rows in top half + 4 rows in bottom half.
    // - Cols: 4 cols in left half + 4 cols in right half.
    //
    // This makes SMEM loads vector-friendly:
    //   a_top/b_left are contiguous float4 loads.
...
#pragma unroll
    for (int kk = 0; kk < kBK; ++kk) {
      const float4 a_top = LoadFloat4(&s_a[kk][a_m_top]);
      const float4 a_bot = LoadFloat4(&s_a[kk][a_m_bot]);
      const float4 b_l = LoadFloat4(&s_b[kk][b_n_left]);
      const float4 b_r = LoadFloat4(&s_b[kk][b_n_right]);
...
int main(int argc, char** argv) {
  ...
  GemmKernelSmemPadBcf<<<grid, block>>>(d_a, d_b, d_c, m, n, k);
