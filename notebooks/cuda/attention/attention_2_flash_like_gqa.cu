/*
CUDA Attention Interview Series (GQA: Grouped Query Attention)

File: attention_2_flash_like_gqa.cu
Focus:
  - Extend attention_1 to GQA:
      Q has Hq heads, but K/V have Hkv heads, with Hq % Hkv == 0.
      Each group of (Hq / Hkv) query heads shares one KV head.
  - Same FlashAttention-style online softmax as attention_1.

Interview tags:
  - Classic:     High
  - Importance:  Extremely High (common in modern LLMs)
  - Frequency:   High

Memorize (recommended if you target LLM inference roles):
  - YES. GQA shows you understand real model architectures.

Build (recommended for RTX 4070 / SM89):
  nvcc -O3 -std=c++17 -lineinfo -arch=sm_89 attention_2_flash_like_gqa.cu \
    -o attention_2_flash_like_gqa
Run:
  ./attention_2_flash_like_gqa [seq_len] [head_dim] [num_q_heads] \
    [num_kv_heads] [causal(0|1)] [device_id]
Example:
  ./attention_2_flash_like_gqa 1024 64 8 2 1 0

Notes:
  - Real LLM inference uses a KV cache and often paged attention.
  - This demo keeps everything contiguous and recomputes full attention.
*/
...
constexpr int kDefaultNumQHeads = 8;
constexpr int kDefaultNumKvHeads = 2;
...
void FillQ(std::vector<float>* q, int num_q_heads, int seq_len, int head_dim) {
  const size_t n = static_cast<size_t>(num_q_heads) *
...
void FillKv(std::vector<float>* k,
            std::vector<float>* v,
            int num_kv_heads,
            int seq_len,
            int head_dim) {
  const size_t n = static_cast<size_t>(num_kv_heads) *
                   static_cast<size_t>(seq_len) *
                   static_cast<size_t>(head_dim);
  k->resize(n);
  v->resize(n);
...
void CpuAttentionGqa(const std::vector<float>& q,
  ...
  for (int hq = 0; hq < num_q_heads; ++hq) {
    const int hk = hq / group_size;
    for (int i = 0; i < seq_len; ++i) {
      double max_s = -INFINITY;
      const size_t q_base =
          (static_cast<size_t>(hq) * static_cast<size_t>(seq_len) +
           static_cast<size_t>(i)) *
          static_cast<size_t>(head_dim);
      for (int j = 0; j < seq_len; ++j) {
        ...
        double dot = 0.0;
        const size_t k_base =
            (static_cast<size_t>(hk) * static_cast<size_t>(seq_len) +
          ...
          const size_t v_idx =
              (static_cast<size_t>(hk) * static_cast<size_t>(seq_len) +
               static_cast<size_t>(j)) *
                  static_cast<size_t>(head_dim) +
              static_cast<size_t>(d);
          out += pj * static_cast<double>(v[v_idx]);
        }
        const size_t o_idx =
            (static_cast<size_t>(hq) * static_cast<size_t>(seq_len) +
             static_cast<size_t>(i)) *
...
__global__ void AttentionKernelFlashLikeGqa(const float* __restrict__ q,
  ...
  if (hq >= num_q_heads || query_i >= seq_len) {
    return;
  }
...
int main(int argc, char** argv) {
  ...
  const int num_q_heads =
      (argc >= 4) ? ParseInt(argv[3], kDefaultNumQHeads) : kDefaultNumQHeads;
  const int num_kv_heads =
      (argc >= 5) ? ParseInt(argv[4], kDefaultNumKvHeads) : kDefaultNumKvHeads;
  const bool causal = (argc >= 6) ? (ParseInt(argv[5], 0) != 0) : true;
  const int device_id = (argc >= 7) ? ParseInt(argv[6], 0) : 0;

  if (seq_len <= 0 || head_dim <= 0 || num_q_heads <= 0 || num_kv_heads <= 0) {
    std::fprintf(stderr, "seq_len, head_dim, heads must be > 0\n");
    return 2;
  }
  if ((num_q_heads % num_kv_heads) != 0) {
    std::fprintf(stderr, "require num_q_heads %% num_kv_heads == 0\n");
    return 2;
  }
  ...
  FillQ(&h_q, num_q_heads, seq_len, head_dim);
  FillKv(&h_k, &h_v, num_kv_heads, seq_len, head_dim);

  std::vector<float> h_o_cpu;
  CpuAttentionGqa(h_q, h_k, h_v, &h_o_cpu, num_q_heads, num_kv_heads, seq_len,
                  head_dim, causal);

  const size_t q_n = static_cast<size_t>(num_q_heads) *
                     static_cast<size_t>(seq_len) *
                     static_cast<size_t>(head_dim);
  const size_t kv_n = static_cast<size_t>(num_kv_heads) *
                      static_cast<size_t>(seq_len) *
                      static_cast<size_t>(head_dim);
  const size_t q_bytes = q_n * sizeof(float);
  const size_t kv_bytes = kv_n * sizeof(float);
  const size_t o_bytes = q_n * sizeof(float);
  ...
  const dim3 grid(static_cast<unsigned int>(seq_len),
                  static_cast<unsigned int>(num_q_heads), 1);
  ...
  AttentionKernelFlashLikeGqa<<<grid, block>>>(d_q, d_k, d_v, d_o, num_q_heads,
  ...
  std::vector<float> h_o_gpu(q_n);
  CUDA_CHECK(cudaMemcpy(h_o_gpu.data(), d_o, o_bytes, cudaMemcpyDeviceToHost));
  ...
  std::printf("num_q_heads:  %d\n", num_q_heads);
  std::printf("num_kv_heads: %d\n", num_kv_heads);
  std::printf("causal:       %d\n", causal ? 1 : 0);
