...
File: softmax_3_masked_softmax.cu
Focus:
  - Add a causal mask: for row r, positions col > r are masked out.
  - This is the exact pattern used by decoder-only LLM attention.
  - Keep warp-reduced reductions + float4 vectorization when possible.

Interview tags:
  - Classic:     Very High
  - Importance:  Very High (LLM attention)
  - Frequency:   Very High

Memorize (recommended):
  - YES. Masked softmax is a top-tier "LLM inference CUDA" interview topic.

Build (recommended for RTX 4070 / SM89):
  nvcc -O3 -std=c++17 -lineinfo -arch=sm_89 softmax_3_masked_softmax.cu \
    -o softmax_3_masked_softmax
Run:
  ./softmax_3_masked_softmax [rows] [cols] [causal(0|1)] [device_id]
Example (causal):
  ./softmax_3_masked_softmax 1024 1024 1 0
Example (no mask):
  ./softmax_3_masked_softmax 1024 1024 0 0

Typical interviewer follow-up after this:
  - "Now fuse this softmax into attention (FlashAttention-style)."
  - "How do you do this in fp16/bf16 while keeping stability?"
*/
...
void CpuSoftmaxCausal(const std::vector<float>& x,
                      std::vector<float>* y,
                      int rows,
                      int cols,
                      bool causal) {
...
__global__ void SoftmaxKernelCausal(const float* __restrict__ x,
  ...
  if (use_vec4) {
    ...
    for (int i = threadIdx.x; i < cols4; i += kBlockSize) {
      ...
      if (!causal || (col0 + 2 <= row)) {
        local_max = fmaxf(local_max, v.z);
      }
      if (!causal || (col0 + 3 <= row)) {
  ...
  if (use_vec4) {
    ...
    for (int i = threadIdx.x; i < cols4; i += kBlockSize) {
      ...
      const float4 v = x4[i];
      float4 out{};
      out.x = (causal && (col0 + 0 > row)) ? 0.0f
                                           : (__expf(v.x - row_max) * inv);
      out.y = (causal && (col0 + 1 > row)) ? 0.0f
                                           : (__expf(v.y - row_max) * inv);
      out.z = (causal && (col0 + 2 > row)) ? 0.0f
                                           : (__expf(v.z - row_max) * inv);
      out.w = (causal && (col0 + 3 > row)) ? 0.0f
                                           : (__expf(v.w - row_max) * inv);
      y4[i] = out;
    }
  } else {
    for (int col = threadIdx.x; col < cols; col += kBlockSize) {
      if (causal && (col > row)) {
        y[row_base + static_cast<size_t>(col)] = 0.0f;
        continue;
      }
...
int main(int argc, char** argv) {
  ...
  CpuSoftmaxCausal(h_x, &h_y_cpu, rows, cols, causal);
  ...
  SoftmaxKernelCausal<<<rows, kBlockSize>>>(d_x, d_y, rows, cols, causal);
