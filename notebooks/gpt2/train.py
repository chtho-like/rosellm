...
class GPTConfig:
    block_size: int = 1024
    vocab_size: int = 50257  # 50000 merges + 256 + 1 <|endoftext|>
    n_layer: int = 12
    n_head: int = 12
    n_embd: int = 768
...
class GPT(nn.Module):
    ...
    def from_pretrained(cls, model_type):
        # Model configuration for different GPT-2 sizes.
        config_args = {
...
if ddp:
    assert torch.cuda.is_available(), "for now only support cuda training in DDP"
...
# Initialize wandb in the master process only
if master_process:
    wandb.init(
        entity="guoqizhou123123-tencent",
        project="GPT",
        config={
            "learning_rate": 6e-4,
            "architecture": "GPT",
...
# model = GPT(GPTConfig())
model = GPT(GPTConfig(vocab_size=50304))  # optimize for 2's power
model.eval()
model.to(device)
...
max_lr = 6e-4  # 3e-4
min_lr = max_lr * 0.1
warmup_steps = 715  # 375M (from GPT-3 paper) / batch_size
max_steps = 19073  # 10B / batch_size = 10B / 524288 = 19073
...
total_batch_size = 2**19  # 524288, ~0.5M tokens
B = 8  # micro batch size
T = 1024  # sequence length
assert (
    total_batch_size % (B * T * ddp_world_size) == 0
), "make sure total_batch_size is divisible by (B * T * ddp_world_size)"
grad_accum_steps = total_batch_size // (B * T * ddp_world_size)  # 64
if master_process:
...
# print(loss)  # initial loss: -ln(1/50257) = 10.82
...
for step in range(max_steps):
    ...
    if step % 100 == 0:
        ...
        if master_process:
            ...
            wandb.log({"val_loss": val_loss_accum}, step=step)
    if step % 100 == 0 or is_last_step:
        ...
        if master_process:
            print(
                f"step {step} | HellaSwag acc: {num_correct_norm} / {num_total} = {acc_norm:.4f}"
            )
            wandb.log({"HellaSwag_acc": acc_norm}, step=step)
    # ============ Generate ==============
    if step % 100 == 0:
        model.eval()
        ...
        for i in range(num_return_sequences):
            ...
            print(f"rank {ddp_rank} sample {i} generated: {decoded}")

        if master_process:
            # Log generated text samples to wandb
            samples_table = wandb.Table(columns=["step", "sample_id", "generated_text"])
            for i in range(num_return_sequences):
                tokens = xgen[i, :max_length].tolist()
                decoded = enc.decode(tokens)
                samples_table.add_data(step, i, decoded)
    ...
    tok_sec = train_loader.B * train_loader.T * grad_accum_steps * ddp_world_size / dt
    if master_process:
        print(
            f"step {step} | loss: {loss_accum:.4f} | lr: {lr:.2e} | norm: {norm:.4f} | "
            f"time: {dt:.2f} ms | tok/sec: {tok_sec:.2f}"
        )
        wandb.log(
            {
