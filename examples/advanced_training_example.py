...
from rosellm.rosetrainer.config import TrainingConfig, PrecisionType
...
from rosellm.rosetrainer.parallelism import initialize_model_parallel, destroy_model_parallel
...
def main():
    ...
    print(f"  Gradient clipping: {config.gradient.clip_type} @ {config.gradient.clip_value}")
    ...
    if config.memory.activation_checkpointing:
        ...
        print(f"Memory saved by checkpointing: {memory_report['total_memory_saved_gb']:.3f} GB")
        print(f"Average recompute time: {memory_report['average_recompute_time_ms']:.1f} ms")
    ...
    print(f"  Average throughput: {perf_report['performance']['samples_per_second']:.1f} samples/sec")
